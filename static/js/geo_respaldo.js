...
function setDashboardData(feature) {
  ...
  if($("#radio_sem0").is(":checked")){
    ...
    $("#cases-dashboard2 #socio.data").html(
      feature == null || feature.properties.socio == null ? "--" : feature.properties.socio
    );
    $("#cases-dashboard3 #emergencias .data").html(
    ...
    $("#cases-dashboard #activos .data").html(
      _prediccionesMapa == null ? "--" : _prediccionesMapa[feature.properties.codigo]
    );
  }
}
...
function ajaxChartRefresh(province = null, canton = null, distrito = null) {
  let url = "getLineChart";
  $("#grafico1").LoadingOverlay("show");
  $.get(
    url,
    { province: province, canton: canton, distrito: distrito },
    function (result) {
      $("#grafico1").html(result["chart"]);
      $("#grafico1").LoadingOverlay("hide");
      setUpGrafico1();
    }
  );
}
...
window.onload = function () {
  ...
  /**
   * Colorea de gris la provincia seleccionada en el select.
  ...
  $("#provincias").on("change", function () {
    ...
    if (selectedProvince != "NONE") {
      ajaxChartRefresh(selectedProvince);
  ...
  $("input:radio[name=radio-group-1-bg]").change(function () {
    ...
    let url = tipo == 1 ? "getGaugeChart" : "getGaugeAnimado";
    $("#gauge1").LoadingOverlay("show");
    $("#Map").LoadingOverlay("show");
    $.get(
      url,
      { tipo: tipo, province: province, canton: canton, distrito: distrito, fecha: _fechaActual },
      function (result) {
...
function seleccionarDistrito(distrito){
  if (_selectedDistrito == distrito) return;
      if (_selectedProvince != null && _selectedCanton != null) {
        ...
        ajaxChartRefresh(_selectedProvince, _selectedCanton, _selectedDistrito);
        let recuperados = $(this).attr("recuperados");
        let activos = $(this).attr("activos");
        let proInfo = $(this).attr("provincia");
        let cantInfo = $(this).attr("canton");
...
/**
 * Actualiza el mapa con la información de la provincia
 ...
 * @param provincia Provincia cuyos datos serán cargados en el gráfico de líneas.
 */
function changeGauge(province = null, canton = null, distrito = null) {
  let url = "getGaugeChart";
  $("#gauge1").LoadingOverlay("show");
  $("#Map").LoadingOverlay("show");
  $.get(
    url,
    {fecha: _fechaActual, province: province, canton: canton, distrito: distrito },
    function (result) {
      $("#gauge1").html(result["chart"]);
      $("#gauge1").LoadingOverlay("hide");
      //setUpGrafico1();
...
/**
 * Ajusta el tamaño y propiedades del gráfico 1 para que sea escalable al cambiar el tamaño del div.
 */
function setUpGrafico1() {
  let grafico1 = $("#grafico1 svg");
  $.each(grafico1, function (index, value) {
    value.removeAttribute("height");
    value.removeAttribute("width");
    value.setAttribute("viewBox", "0 0 550 220");
  });
  grafico1.css("max-width", "650px");
  grafico1.css("max-width", "570px");
  grafico1.css("overflow", "hidden");
}
